...
# "Main"
    # SPECIAL FOR STF45: in this script (etherwise a copy of fit_comparison) we correct the probable goniometer motor
    # blocking in the data taking by recentering the data around theta_c + theta_bending
    # Data VRAM center (where the two peaks are 50%/50%): 169.432 murad
    # Theoretical theta_c + theta_bending: 153.875 murad
    # Difference: 15.557 murad
...
particle_energy = crystal_params[crystal_name]['particle_energy[GeV]'] * 1e9 # [eV]
...
while (cur_slice < to_slice):
    ...
    #
    # SPECIAL FOR STF45: in this script correct the probable goniometer motor
    # blocking in the data taking by recentering the data around theta_c + theta_bending
    # Data VRAM center (where the two peaks are 50%/50%): 169.5 murad
    # Theoretical theta_c + theta_bending: 153.5 murad
    # Difference: 16 murad
    distribution = []
    for i in range(len(y)):
        posizione_col = x[i] # Here we subtract it
        altezza_col = y[i]
