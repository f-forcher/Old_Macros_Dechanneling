...
y_sigmasVR_exp = [np.sqrt(xx) for xx in sigma2s_VR_exp.values()]
y_sigmasAM_exp = [np.sqrt(xx) for xx in sigma2s_AM_exp.values()]
...
y_sigmasVR_sim = [np.sqrt(xx) for xx in sigma2s_VR_sim.values()]
y_sigmasAM_sim = [np.sqrt(xx) for xx in sigma2s_AM_sim.values()]
...
plt.plot(
    ...
    y_sigmasVR_exp,
    linestyle="solid",
    label="Experimental VR Sigmas",
    color='Blue')
plt.plot(
    x_AM_sim,
    y_sigmasVR_sim,
    linestyle="dashed",
    label="Simulation VR Sigmas",
    color='BlueViolet')
plt.plot(
    x_AM_exp,
    y_sigmasAM_exp,
    linestyle="solid",
    label="Experimental AM Sigmas",
    color='Crimson')
plt.plot(
    x_AM_sim,
    y_sigmasAM_sim,
    linestyle="dashed",
    label="Simulation AM Sigmas",
    color='Darkgreen')
# Plot the characteristic angles of the transition region
plt.axvline(x=or_sign*(theta_bending), linestyle="dashed", color='Crimson')  # TODO
plt.axvline(x=or_sign*(theta_bending + theta_c), linestyle="dashed")  #TODO
