...
void torsion_histo(){
  ...
  //TString outputfile = "recoDataSimple_415.notorsion.histo.root";
  TString outputfile = "recoDataSimple_410.notorsion.histo.root";
  ...
  */
  ...
  Double_t ThetaInmin = 0.0;

  double init_scan = 4714940.0;	//410
  //double init_scan = 4714900.0;  //415
  ...
  TH1D *d0_y_h = new TH1D("d0_y_h", " Impact point at crystal y ",1000,-10.,10.);
  ...
  TH1D * dTheta_x = new TH1D("dTheta_x","dTheta_x",600,-200.,400.);
  TH3D * dTheta_x_vs_Impact_x_vs_Impact_y = new TH3D("dTheta_x_vs_Impact_x_vs_Impact_y","dTheta_x_vs_Impact_x_vs_Impact_y", 600,-200.,400.,60,-30., 30.,100,-3.,3.);

TH3D * dTheta_x_vs_Impact_x_vs_Impact_y_w = new TH3D("dTheta_x_vs_Impact_x_vs_Impact_y_w","dTheta_x_vs_Impact_x_vs_Impact_y_w", 600,-200.,400.,60,-30., 30.,100,-3.,3.);

  TH2D * dTheta_x_vs_Impact_x = new TH2D("dTheta_x_vs_Impact_x","dTheta_x_vs_Impact_x",100,-50.,50., 600,-200.,400.);
  dTheta_x_vs_Impact_x->GetXaxis()->SetTitle("Impact #theta_{x} [#murad]");
  dTheta_x_vs_Impact_x->GetYaxis()->SetTitle("#theta_{x} [#murad]");

  TH2D * dTheta_x_vs_Impact_x_w = new TH2D("dTheta_x_vs_Impact_x_w","dTheta_x vs Impact_x weight",100,-50.,50., 600,-200.,400.);
  dTheta_x_vs_Impact_x_w->GetXaxis()->SetTitle("Impact Angle #theta_{x} [#murad]");
  dTheta_x_vs_Impact_x_w->GetYaxis()->SetTitle("#Delta#theta_{x} [#murad]");
  ...
  //TH2D * dTheta_x_vs_Impact_x_nocut = new TH2D("dTheta_x_vs_Impact_x_nocut" , " dTheta_x_vs_Impact_x_nocut " , 1400,-400.,1000.,500., -50., 50.);
...
//  Maxd0y = 2.2778;
  ...
  for (Int_t i = 0; i < nentries; i++) {
    ...
    d0_y_h->Fill(tracks.thetaIn_x[9]);
    ...
    if(ntracks==1 && cutd0x && cutd0y) {
      ...
      dTheta_x_vs_Impact_x_vs_Impact_y->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6,gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan,tracks.thetaIn_x[9]);
      dTheta_x_vs_Impact_x->Fill(gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan+8.9,(tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);
  ...
  for (Int_t i = 0; i < 100; i++) {
    ...
    y_min = yaxis->FindBin(-200);
    y_max = yaxis->FindBin(400);

    count[i] =  dTheta_x_vs_Impact_x->Integral(x_min,x_max,y_min,y_max);
    cout << i <<" " << count[i] << endl;
  }
  ...
  for (Int_t i = 0; i < 100; i++) {
    for (Int_t j = 0; j < 600; j++) {
      ...
      value = dTheta_x_vs_Impact_x->GetBinContent(x,y);

      dTheta_x_vs_Impact_x_w->SetBinContent(x,y,((value/count[i])*10000));
  ...
  //  dTheta_x_vs_Impact_x_w->Write();

  myfile.close();


  TCanvas *c1 = new TCanvas();
  dTheta_x_vs_Impact_x_vs_Impact_y->Draw("colz");
