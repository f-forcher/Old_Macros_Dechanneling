...
void torsion_histo_corr(){
  ...
  //  TString fileName("/Volumes/dfs/Experiments/UA9/Data-Test_beams/2010_09_16\,_H8_re-recodata/recoDataSimple_415_1-37");
  ...
  TString outputfile = "recoDataSimple_410.torsion.correction.histo.root";

  fileName += ".root";

  // Output file name
  myfile.open("recoDataSimple_410.torsion.correction.histo.dat");
  ...
  */
  ...
  Double_t ThetaInmin = -10.0;

  double init_scan = 4714940.0;	//410
  //double init_scan = 4714950.0;  //415
  //double init_scan = 4712980.0;
  int chentres = 4889340;
  double a =8.85508;
  double b = -7.49946;
  ...
  TH1D * dTheta_x_10k = new TH1D("dTheta_x_10k","dTheta_x_10k",400,-200.,200.);
  TH1D * dTheta_x_100k = new TH1D("dTheta_x_100k","dTheta_x_100k",400,-200.,200.);
  TH1D * dTheta_x_5bin = new TH1D("dTheta_x_5bin","dTheta_x_bin5",80,-200.,200.);
  TH1D * dTheta_x_5bin_10k = new TH1D("dTheta_x_5bin_10k","dTheta_x_5bin_10k",80,-200.,200.);
  TH1D * dTheta_x_5bin_100k = new TH1D("dTheta_x_5bin_100k","dTheta_x_5bin_100k",80,-200.,200.);
  ...
  TH2D * dTheta_x_vs_Impact_x = new TH2D("dTheta_x_vs_Impact_x","dTheta_x vs Impact_x",100,-50.,50., 600,-200.,400.);
  ...
  TH2D * dTheta_x_vs_Impact_x_cor = new TH2D("dTheta_x_vs_Impact_x_cor","dTheta_x vs Impact_x tor Corr",100,-50.,50., 600,-200.,400.);
  ...
  TH1D * h5 = new TH1D("h5","h5", 600,-200.,400.);


  TH1D * h10 = new TH1D("h10","h10", 600,-200.,400.);
  ...
  int cutd0y(0), cutd0x(0), angle_cut(0), angle_cut2(0), cutd0y_new(0);
  ...
  for (Int_t i = 0; i < nentries; i++) {
    ...
    /*    dTheta_x->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);
    dTheta_x_5bin->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);

    if (i < 10000) {
      dTheta_x_10k->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);
      dTheta_x_5bin_10k->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);
    }

    if (i < 100000) {
      dTheta_x_100k->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);
      dTheta_x_5bin_100k->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);
    }
    */
    ...
    if(ntracks==1 && cutd0x && cutd0y) {
      ...
      //  if (cutd0x && cutd0y) {
      //	count_d0xcut++;
      if ((gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan-((a*tracks.thetaIn_x[9])+b)) < 5. && (gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan-((a*tracks.thetaIn_x[9])+b)) > -5.) angle_cut=1;

      if (angle_cut) {h5->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);}

      if ((gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan-((a*tracks.thetaIn_x[9])+b)) < 10. && (gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan-((a*tracks.thetaIn_x[9])+b)) > -10.) angle_cut2=1;

      if (angle_cut2) {h10->Fill((tracks.thetaIn_x[2] - tracks.thetaIn_x[0])*1e6);}
      ...
      myfile << gPos.x+(tracks.thetaIn_x[0]*1e6)-init_scan-((a*tracks.d0_y[0])+b) << " " << tracks.thetaIn_x[1]*1e6 <<" "<<gPos.x+(tracks.thetaIn_x[2]*1e6)-init_scan-((a*tracks.d0_y[0])+b)<< " " << tracks.thetaIn_x[3]*1e6 << endl;
  ...
  //  dTheta_x_100k->Write();
  //  dTheta_x_5bin->Write();
  //  dTheta_x_5bin_10k->Write();
  //  dTheta_x_5bin_100k->Write();
  ...
  dTheta_x_5bin_10k->Draw("E");
  TCanvas *c21 = new TCanvas();
  dTheta_x_10k->Draw("E");

  TCanvas *c12 = new TCanvas();
  dTheta_x_5bin_100k->Draw("E");
  TCanvas *c22 = new TCanvas();
  dTheta_x_100k->Draw("E");
  */
