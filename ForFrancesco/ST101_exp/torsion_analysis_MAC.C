...
void torsion_analysis() {
  ofstream myfile;
  //Apre file dati
  TFile * in_file = new TFile("recoDataSimple_2262.torsion.histo.root");
   
  TString outputfile = "recoDataSimple_2262.torsion.graph.root";
  ...
  */
  ...
  for (int i = 0; i < yn; i++) {
	...
	if (int_am[i][j] > 60) { 
	  ...
	  TF1 *g2    = new TF1("g2","gaus",40.,100.);
	  g2->SetLineColor(kRed); 
	  g2->SetParLimits(1,20.,60.);
	  g2->SetParLimits(2,5.,15.);
	  ...
	  if (mean_ch[i][j] < 80. && mean_ch[i][j] > 40.) mean_bound = 1;
	
	  if(bound && mean_bound){

	    int_ch[i][j] = sqrt(2.*TMath::Pi())*sigma_ch[i][j]*cost_ch[i][j];
	  
	    if (int_ch[i][j] > int_am[i][j]) {
	      int bmin_ch = axis->FindBin(20);
	      int bmax_ch = axis->FindBin(100); 
	...
	eff_int[i][j] = 0.;
	cout<<"control no event"<< eff_int[i][j]<<endl;
      }
	...
	int bmin_ch = axis->FindBin(23);
	int bmax_ch = axis->FindBin(60); 
  ...
  /*
  TH1D *hs= h->ProjectionX("z slice",30,30,49,49);
  ...
  */
  ...
  TH2D * efficiency_vs_impact_x_vs_Impact_y = new TH2D("efficiency_vs_impact_x_vs_Impact_y","efficiency_vs_impact_x_vs_Impact_y",100,-3.,3.,60,-30., 30.);
  ...
  */ 
  ...
  Double_t r_y[200];
  Double_t err_y[200];
  Double_t counter[100];
  Double_t eff_int_turn[200][200];

  
  //TVectorD mean_x(zn+1);
  //TVectorD sigma_x(zn+1);
  Double_t mean_x[200];
  Double_t sigma_x[200];
  Double_t sigma_eff[200];
  Double_t stderr[200];
  Double_t mean[200];
  Double_t mean_w[200];
  Double_t somma[200];
  Double_t sum[200];
  Double_t prod[200];
  Double_t sum_pos[200];



  Double_t r_x[200][200];
  int count(0);  
  ...
  TF1 * flin = new TF1("flin","pol1",-1.5,1.5);
  ...
  Double_t **eff_clean;
  Double_t **r_x_clean;
  eff_clean = (double **) malloc(100*sizeof(double *));
  r_x_clean = (double **) malloc(100*sizeof(double *));
  ...
  */
  ...
  Double_t sum_test[200];
  Double_t err_test[200];
