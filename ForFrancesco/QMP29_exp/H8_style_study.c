...
void H8_style_study(){
  ...
  filename="recoDataSimple_1197.torsion.correction.histo.dat";
  nhead=0;
  ...
  TFile *f=new TFile("QMP29Rob_Exp.root","new");
  ...
  TH2F *hH8=new TH2F("hH8","histo H8 style",60,-30,30,230,-30,200);

  TH1D *h5= new TH1D("h5","5urad cut",230,-30,200);
  h5->GetXaxis()->SetTitle("#Delta#theta_{x} [#murad]");
  //h5->Rebin(4);                                                                                                                                                                                           

  TH1D *h10= new TH1D("h10","10urad cut",230,-30,200);
  ...
  for(i=0;i<dim-nhead;i++){
      ...
      if(xp_i[i]>-5.0 && xp_i[i]<5.0)
	h5->Fill(xp_o[i]-xp_i[i]);

      if(xp_i[i]>-10.0 && xp_i[i]<10.0)
	h10->Fill(xp_o[i]-xp_i[i]);

      if(xp_i[i] == xp_i[i-1] || (xp_o[i]-xp_i[i]) == (xp_o[i-1]-xp_i[i-1]) )
	cout<<"same x' or kick"<<xp_i[i]<<" "<<xp_i[i-1]<<endl;

      if(xp_i[i] == xp_i[i-1] && (xp_o[i]-xp_i[i]) == (xp_o[i-1]-xp_i[i-1]) )
	cout<<"same x' and kick"<<endl;
  ...
  for(i=0;i<dim-nhead;i++){
    ...
    if(xp_i[i]>(-20.0) && xp_i[i]<(-10.0))
      in0->Fill(xp_o[i]-xp_i[i]);

    if(xp_i[i]>(10.0) && xp_i[i]<(20.0))
