...
// questo programma genera : plot zona transizione e popolazioni CH, DC e TR
void population() {
  ...
  /* 
  //Salva plot
  TString outputfile = "recoDataSimple_415.transiction.graph.root"; 
  */

  TFile * in_file = new TFile("QMP32Rob_Exp.root");
  TString outputfile = "QMP32Rob.population_Exp_new.root"; 
  ...
  //test su picco channeling inizializzazione dechanneling
  Double_t par_ch[3]=0;
  ...
  dech_eff = (double *) malloc(n*sizeof(double));
  ...
  eff_de = (double *) malloc(n*sizeof(double)); 
  eff_ch = (double *) malloc(n*sizeof(double)); 
  eff_tr = (double *) malloc(n*sizeof(double));
  int_ch = (double *) malloc(n*sizeof(double));
  int_de = (double *) malloc(n*sizeof(double));
  int_tr = (double *) malloc(n*sizeof(double));
  int_tot = (double *) malloc(n*sizeof(double));
  err_tr = (double *) malloc(n*sizeof(double));
  err_ch = (double *) malloc(n*sizeof(double));
  err_de = (double *) malloc(n*sizeof(double));
  err_tot = (double *) malloc(n*sizeof(double));
  err_eff_de = (double *) malloc(n*sizeof(double));
  err_eff_ch = (double *) malloc(n*sizeof(double));
  err_eff_tr = (double *) malloc(n*sizeof(double));
  dech_len = (double *) malloc(n*sizeof(double));
  ...
  // generazione punti dei plot 
  for(int i=0;i<n;i++) {


    //fit e definizione bordi dei picchi
    h_slice[i]->Fit(g1,"R+");
    h_slice[i]->Fit(g2,"R+");
    ...
    Double_t ch_min = par_ch[1]-(par_ch[2]*3);
    Double_t am_max = par_am[1]+(par_am[2]*3);
    Double_t ch_max = par_ch[1]+(par_ch[2]*3);
    Double_t am_min = par_am[1]-(par_am[2]*3);


    //fit dechannling -> lunghezza in funzione angolo
    TF1 *de = new TF1("de",myfunction,am_max,ch_min,2);
    de->SetLineColor(kGreen);
    de->SetParameters(zero,uno);
    h_slice[i]->Fit(de,"R+");

    de->GetParameters(&par_de[0]);

    dech_len[i] = R*(par_de[1]*1e-3);

    //errors on transiction
    //h_slice[i]->GetXaxis()->SetRange(-20.,10.);
    //tr_mean[i] = h_slice[i]->GetMean();
    tr_sigma[i] = h_slice[i]->GetRMS(1)/TMath::Sqrt(h_slice[i]->Integral());
    // h_slice[i]->GetXaxis()->SetRange();

    cout << i <<" " <<err_tr[i]<< " " << am_min << " " << am_max << endl;   

 

    // Integrali per le popolazioni

    TAxis *axis = h_slice[i]->GetXaxis();
    de_min = axis->FindBin(am_max);
    de_max = axis->FindBin(ch_min);

    bch_min = axis->FindBin(ch_min);
    bch_max = axis->FindBin(ch_max);

    bam_min = axis->FindBin(am_min);


    tot_min = axis->FindBin(-200);
    tot_max = axis->FindBin(400);
    /*
    int_ch[i] = h_slice[i]->IntegralAndError(bch_min,bch_max,err_ch[i]);
    int_tot[i] = h_slice[i]->IntegralAndError(tot_min,tot_max,err_tot[i]);
    int_de[i] = h_slice[i]->IntegralAndError(de_min,de_max,err_de[i]);
    int_tr[i] = h_slice[i]->IntegralAndError(bam_min,de_min,err_tr[i]);
    */

    int_ch[i] = h_slice[i]->Integral(bch_min,bch_max);
    int_tot[i] = h_slice[i]->Integral(tot_min,tot_max);
    int_de[i] = h_slice[i]->Integral(de_min,de_max);
    int_tr[i] = h_slice[i]->Integral(bam_min,de_min);


    //cout<<"channeled event" <<int_ch[i][j]<<endl;
    //cout << int_ch[i] <<" "<<err_ch[i] << endl;
    
    eff_de[i] = int_de[i]/int_tot[i]*100.0;//274891;//(int_tot[i]);
    err_eff_de[i] = eff_de[i]*TMath::Sqrt(TMath::Power((err_tot[i]/int_tot[i]),2)+TMath::Power((err_de[i]/int_de[i]),2));

    eff_ch[i] = int_ch[i]/int_tot[i];//274891;//(int_tot[i]);
    err_eff_ch[i] = eff_ch[i]*TMath::Sqrt(TMath::Power((err_tot[i]/int_tot[i]),2)+TMath::Power((err_ch[i]/int_ch[i]),2));

    eff_tr[i] = int_tr[i]/int_tot[i];//274891;//(int_tot[i]);
    err_eff_tr[i] = eff_tr[i]*TMath::Sqrt(TMath::Power((err_tot[i]/int_tot[i]),2)+TMath::Power((err_tr[i]/int_tr[i]),2));
    ...
    /*
    cout << int_ch[i] <<" "<<err_ch[i] << endl;
    cout << int_de[i] <<" "<<err_de[i] << endl;
    cout << eff_de[i] <<" "<<err_eff_de[i] << endl;
    */	
  }
  ...
  TCanvas *cdech = new TCanvas("cdech","cdech",1100,1100);
  TGraph *gr_len = new TGraph(n,imp_ang,dech_len);
  //TGraphErrors *de_len = new TGraphErrors(n,imp_ang,de_len_mean,err_ang,tr_sigma);
  gr_len->SetTitle("Dechanneling Length vs. Impact x angle");
  gr_len->GetXaxis()->SetTitle("#theta_{x}^{in} [#murad]");
  gr_len->GetYaxis()->SetTitle("Dechanneling leght [mm]");
  gr_len->Draw("ap");

  TF1 *fdc=new TF1("fdc","x*x-x*x*x*x",-1.5,1.5);
  ...
  TGraphErrors *gr = new TGraphErrors(n,imp_ang,tr_mean,err_ang,tr_sigma);
  //gr->SetTitle("Peak position in transiction zone");
  gr->GetXaxis()->SetTitle("#theta_{x}^{in}/#theta_{c}");
  ...
  gr->GetXaxis()->SetRangeUser(-1.5,1.5);
  gr->Draw("ap");
  /*
  TPaveText *legT=new TPaveText(0.65,0.85,0.95,0.95,"NDC");
  legT->SetFillColor(10);
  legT->SetTextFont(132);
  
  legT->AddText("Mean deflection in the transition AM #rightarrow VR regime");
  legT->SetTextSize(0.035);
  legT->Draw();
  */
  ...
  TLine *am = new TLine(-1.5,tr_mean[0],1.5,tr_mean[0]);
  am->SetLineColor(kRed);
  am->SetLineStyle(9);


  TLine *vr = new TLine(-1.5,tr_mean[n-2],1.5,tr_mean[n-2]);
  ...
  leg = new TLegend(0.53,0.85,0.95,0.97);
  leg->SetFillColor(10);
  leg->SetTextFont(132);
  leg->SetTextSize(0.035);
  //leg->SetMargin(1);
  leg->SetHeader("Mean deflection in the transition AM #rightarrow VR regime");
  leg->AddEntry(am,"Mean deflection in full AM","l");
  leg->AddEntry(vr,"Mean deflection in full VR","l");

  leg->Draw();

  TCanvas *cde = new TCanvas("cde","cde",1100,1100);
  cde->SetFillColor(10);
  cde->SetGridx(1);
  cde->SetGridy(1);
  //TGraph *grd = new TGraph(n,imp_ang,eff_de);
  TGraphErrors *grd = new TGraphErrors(n,imp_ang,eff_de,err_ang,err_eff_de);
  ///grd->SetTitle("Dechanneling Population (normalized to total events)");
  grd->GetXaxis()->SetTitle("#theta_{x}^{in}/#theta_{c}");
  grd->GetYaxis()->SetTitle("Dechanneled protons [%]");
  grd->GetXaxis()->SetTitleFont(132);
  grd->GetYaxis()->SetTitleFont(132);
  grd->GetXaxis()->SetLabelFont(132);
  grd->GetYaxis()->SetLabelFont(132);
  grd->SetMarkerColor(kRed);
  grd->SetMarkerStyle(20);
  grd->GetXaxis()->SetRangeUser(-1.5,1.5);
  grd->Draw("ap");


  TCanvas *cch = new TCanvas("cch","cch",1100,1100);
  //TGraph *grd = new TGraph(n,imp_ang,eff_de);
  TGraphErrors *grc = new TGraphErrors(n,imp_ang,eff_ch,err_ang,err_eff_ch);
  grc->Draw("ap");
  grc->SetTitle("Channeling Population (normalized to total events)");
  grc->GetXaxis()->SetTitle("#theta_{x}^{in}/#theta_{c}");
  grc->GetYaxis()->SetTitle(" #channeled_events / #total_events");


  TCanvas *ctr = new TCanvas("ctr","ctr",1100,1100);
  //TGraph *grd = new TGraph(n,imp_ang,eff_de);
  TGraphErrors *grt = new TGraphErrors(n,imp_ang,eff_tr,err_ang,err_eff_tr);
  grt->Draw("ap");
  grt->SetTitle("Transition Region Population (normalized to total events)");
  grt->GetXaxis()->SetTitle("#theta_{x}^{in}/#theta_{c}");
  grt->GetYaxis()->SetTitle(" #transition_events / #total_events");


 TLine *am1 = new TLine(-50,20,50,20);
  am1->SetLineColor(kRed);
  am1->SetLineStyle(9);


  TLine *vr1 = new TLine(-50,-50.,50,-50.);
  vr1->SetLineColor(kRed);
  vr1->SetLineStyle(9);
