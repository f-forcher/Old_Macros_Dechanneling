 ...
 */
std::unique_ptr<std::map<std::string, Double_t> > calculate_cystal_params_from_fit(
		Double_t thickness, Double_t bending_angle, Double_t bending_angle_err,
		Double_t slopeDc, Double_t slopeDc_err) {
	...
	unique_ptr<map<string, Double_t> > mapc = std::make_unique<
			map<string, Double_t> >();
	...
	auto Rc_err = (thickness * MILLI_) / pow(bending_angle * MICRO_, 2)
			* bending_angle_err * MICRO_;
	...
	auto dechanneling_lenght_err = sqrt(
			pow(Rc * thetad_err * MICRO_, 2)
...
void mia_dech(std::string nome_cristallo,
	...
	DBG(
			clog << " meanAm5: " << meanAm5 << endl << " sigmaAm5: " << sigmaAm5
					<< endl
			;, ;)
	...
	DBG(
			clog << " meanCh5: " << meanCh5 << endl << " sigmaCh5: " << sigmaCh5
					<< endl
			;, ;)
	...
	TF1* fDech5 = new TF1("fDech5", "expo", meanAm5 + 3 * sigmaAm5,
			meanCh5 - 3 * sigmaCh5);
	TF1* fDech10 = new TF1("fDech10", "expo", meanAm10 + 3 * sigmaAm10,
			meanCh10 - 3 * sigmaCh10);

	DBG(
			clog << "xmindech: " << meanAm5 + 3 * sigmaAm5 << " xmaxdech: "
					<< meanCh5 - 3 * sigmaCh5 << endl
			;, ;)
	...
	DBG(
			std::clog << "slopeDc5: " << slopeDc5 << " +- " << slopeDc5_err
					<< std::endl
			; , ;)
	DBG(
			std::clog << "slopeDc10: " << slopeDc10 << " +- " << slopeDc10_err
					<< std::endl
			; , ;)
	...
	//Thickness [mm]
	auto thickness =
			dati_cristalli_orig.at(nome_cristallo)[(int) FieldCrystalDataTable::thickness];
	...
	auto map5 = *calculate_cystal_params_from_fit(thickness, bending_angle5,
			bending_angle5_err, slopeDc5, slopeDc5_err);
	auto map10 = *calculate_cystal_params_from_fit(thickness, bending_angle10,
			bending_angle10_err, slopeDc10, slopeDc10_err);

	DBG(
			std::clog << "dechanneling_lenght5: " << map5["dechanneling_lenght"]
					<< std::endl
			; , ;)
	DBG(
			std::clog << "dechanneling_lenght5_err: "
					<< map5["dechanneling_lenght_err"] << std::endl
			; , ;)
	DBG(
			std::clog << "dechanneling_lenght10: "
					<< map10["dechanneling_lenght"] << std::endl
			; , ;)
	DBG(
			std::clog << "dechanneling_lenght10_err: "
					<< map10["dechanneling_lenght_err"] << std::endl
			; , ;)
...
//									 "+ gaus(7)", meanAm5 - 4*sigmaAm5, meanCh5 + 4*sigmaCh5);
	...
	DBG(
			std::clog << "slopeDc_tot5: " << slopeDc_tot5 << " +- "
					<< slopeDc_tot5_err << std::endl
			; , ;)

	//TF1* fDech10_2  = new TF1("fDech10_2", "expo(3)", meanAm10+sigmaAm10, meanCh10-sigmaCh10);
	TF1* fTot10 = new TF1("fTot10",
			"gaus(0) + expo(3) * ( x > [5] && x < [6] ) + gaus(7)",
			meanAm10 - 4 * sigmaAm10, meanCh10 + 4 * sigmaCh10);
	...
	DBG(
			std::clog << "slopeDc_tot10: " << slopeDc_tot10 << " +- "
					<< slopeDc_tot10_err << std::endl
			; , ;)
	...
	auto map5_tot = *calculate_cystal_params_from_fit(thickness,
			bending_angle5_tot, bending_angle5_tot_err, slopeDc_tot5,
			slopeDc_tot5_err);
	auto map10_tot = *calculate_cystal_params_from_fit(thickness,
			bending_angle10_tot, bending_angle10_tot_err, slopeDc_tot10,
			slopeDc_tot10_err);

	DBG(
			std::clog << "dechanneling_lenght5 tot: "
					<< map5_tot["dechanneling_lenght"] << std::endl
			; , ;)
	DBG(
			std::clog << "dechanneling_lenght5_err tot: "
					<< map5_tot["dechanneling_lenght_err"] << std::endl
			; , ;)
	DBG(
			std::clog << "dechanneling_lenght10 tot: "
					<< map10_tot["dechanneling_lenght"] << std::endl
			; , ;)
	DBG(
			std::clog << "dechanneling_lenght10_err tot: "
					<< map10_tot["dechanneling_lenght_err"] << std::endl
			; , ;)
	...
	crystal_calctot_ref[(int) FieldCrystalDataTable510::bending_angle5] =
			bending_angle5_tot;
	crystal_calctot_ref[(int) FieldCrystalDataTable510::bending_angle5_err] =
			bending_angle5_tot_err;
	crystal_calctot_ref[(int) FieldCrystalDataTable510::bending_angle10] =
			bending_angle10_tot;
	crystal_calctot_ref[(int) FieldCrystalDataTable510::bending_angle10_err] =
			bending_angle10_tot_err;
	crystal_calctot_ref[(int) FieldCrystalDataTable510::raggio_curvatura5] =
			map5_tot["Rc"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::raggio_curvatura5_err] =
			map5_tot["Rc_err"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::raggio_curvatura10] =
			map10_tot["Rc"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::raggio_curvatura10_err] =
			map10_tot["Rc_err"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::thickness] = thickness;
	crystal_calctot_ref[(int) FieldCrystalDataTable510::slopeDc5] =
			map5_tot["slopeDc"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::slopeDc5_err] =
			map5_tot["slopeDc_err"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::slopeDc10] =
			map10_tot["slopeDc"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::slopeDc10_err] =
			map10_tot["slopeDc_err"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::dechanneling_lenght5] =
			map5_tot["dechanneling_lenght"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::dechanneling_lenght5_err] =
			map5_tot["dechanneling_lenght_err"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::dechanneling_lenght10] =
			map10_tot["dechanneling_lenght"];
	crystal_calctot_ref[(int) FieldCrystalDataTable510::dechanneling_lenght10_err] =
			map10_tot["dechanneling_lenght_err"];
