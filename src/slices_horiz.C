...
void slices_horiz(std::string nome_cristallo, std::string exp_or_sim, Double_t cut1, Double_t cut2, TH1D*& hist, std::map<std::string, double> otherparams, bool makepng) {
...
//	/* X-dimension */2, -40, 40 );
	...
	//Name the image
	stringstream ss;
	ss << "SlicesOriz_" << cut1 << "_" << cut2 << "_" << nome_cristallo;
	...
	histogram->Rebin( 1 );
	histogram->GetXaxis()->SetRangeUser(otherparams.at("lowXrange_slicesOriz"),otherparams.at("highXrange_slicesOriz"));
	...
	int lowXrange;
	int highXrange;
	if (otherparams.find("key") != otherparams.end() ) {
		lowXrange = -40;
		highXrange = 30;
	} else {
		lowXrange  = otherparams.at("lowYrange_slices");
		highXrange = otherparams.at("highYrange_slices");
	}
	auto minbinofrange = h2D->GetXaxis()->FindBin(lowXrange);
	auto maxbinofrange = h2D->GetXaxis()->FindBin(highXrange);
	auto nbinshist = h2D->GetXaxis()->GetLast();
